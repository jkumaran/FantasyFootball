// Content script for ESPN Fantasy Draft Rooms
(() => {
  console.log('[DraftBridge] ESPN Fantasy Draft Observer Active');
  ...
  function createStatusPill() {
    ...
    pillEl.innerHTML = `
      ...
      <span>🔴 Cameron Bridge: ESPN Draft Sync Active</span>
    `;
  ...
  function scanDraftTable() {
    ...
    const selectors = [
      '.draft-history-feed tr',
      '.draft-history tr',
      '.draft-recap-table tr',
      'div[data-testid="draft-history-item"]',
      '.draft-selection',
      '.player-selection'
    ];
    ...
    elements.forEach(row => {
      try {
        ...
        let pos = null;
        const posMatch = text.match(/\b(QB|RB|WR|TE|DST|D\/ST|K)\b/i);
        if (posMatch) pos = posMatch[1].toUpperCase().replace('D/ST', 'DST');
        ...
        console.log('[DraftBridge] Detected ESPN Pick:', payload);
