export function optimizeWeeklyLineup(state) {
  ...
  const evaluatedPlayers = userTeamPlayers.map(p => {
    let effectiveScore = p.projectedPts / 17;
  ...
  if (startingFlex.length > 0) startersSet.add(startingFlex[0].id);
  ...
  if (scoreDiff >= 8) {
    ...
    strategyReason = `🛡️ You are projected to win by +${Math.round(scoreDiff)} pts. Use CONSERVATIVE mode to secure high floor.`;
  } else {
    recommendedStrategy = 'AGGRESSIVE';
    strategyReason = `⚡ Underdog / Tight Matchup (${Math.round(scoreDiff)} pts diff). Use AGGRESSIVE mode to maximize ceiling upside!`;
  }
